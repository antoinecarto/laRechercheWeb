...
const sketchPsychedelic = (p) => {
  ...
  p.draw = () => {
    ...
    p.background(p.frameCount % 360, 80, 80);
    p.fill((p.frameCount * 2) % 360, 100, 100);
    p.ellipse(
      p.width / 2 + p.sin(p.frameCount * 0.05) * 200,
      p.height / 2 + p.cos(p.frameCount * 0.05) * 200,
      150
    );
  };
