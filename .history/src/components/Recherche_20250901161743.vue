...
// Psychédélique
// Psychédélique
const sketchPsychedelic = (p) => {
  let t = 0;

  p.setup = () => {
    // Même dimension que le sketch normal (800 x 400)
  ...
  p.draw = () => {
    ...
    for (let x = 0; x < p.width; x++) {
      for (let y = 0; y < p.height; y++) {
        let r = 128 + 128 * Math.sin(x * 0.01 + t);
        let g = 128 + 128 * Math.sin(y * 0.01 + t + 2);
        let b = 128 + 128 * Math.sin((x + y) * 0.01 + t + 4);
    ...
    // Afficher le texte en jaune si le texte est chargé et l’animation a commencé
    if (isTextLoaded.value && hasStarted.value) {
      const textSizeValue = Math.min(p.width / 12, 64);
      p.textSize(textSizeValue);
      p.fill(255, 255, 0); // jaune
      ...
      if (index.value < words.value.length) {
        ...
        if (index.value < words.value.length - 1) {
          ...
          p.fill(255, 255, 100, 150); // légèrement transparent
    ...
    // Affichage du message avant le lancement
    if (!hasStarted.value) {
      p.fill(255, 255, 0); // jaune
      p.textSize(28);
      p.text(
  ...
  p.windowResized = () => {
    // Même resize que le sketch normal
...
const toggleSketch = () => {
  isPsychedelic.value = !isPsychedelic.value;
  if (isPsychedelic.value) {
    loadSketch(sketchPsychedelic);
  } else {
    loadSketch(sketch);
  }
};
