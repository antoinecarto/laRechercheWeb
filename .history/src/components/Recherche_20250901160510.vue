  ...
  <div>
    ...
    <div class="p5-container">
      ...
      <div class="controls" v-if="isTextLoaded">
        ...
        <button @click="toggleSketch" class="btn-mode">
          {{ isPsychedelic ? "Retour au texte" : "Mode psychédélique" }}
        </button>
      </div>
...
// Psychédélique
const psychedelicSketch = (p) => {
  p.setup = () => {
    p.createCanvas(
      p5Container.value?.clientWidth || 800,
      p5Container.value?.clientHeight || 400
    );
  ...
  p.windowResized = () => {
    const cw = p5Container.value?.clientWidth || window.innerWidth;
    const ch = p5Container.value?.clientHeight || window.innerHeight;
    p.resizeCanvas(cw, ch);
  };

  p.draw = () => {
    const t = p.millis() * 0.00025;
    const hueBase = (p.sin(t * 2.0) * 60 + 180) % 360;

    // couche de bruit coloré (tiles)
    const tile = 8;
    for (let y = 0; y < p.height; y += tile) {
      for (let x = 0; x < p.width; x += tile) {
        const nx = x * 0.002 + t * 0.3;
        const ny = y * 0.002 - t * 0.2;
        const n = p.noise(nx, ny);
        const hue = (hueBase + n * 240) % 360;
        const sat = 80 + 20 * p.sin((x + y + t * 300) * 0.01);
        const bri = 60 + n * 40;
        p.fill(hue, sat, bri, 10);
        p.rect(x, y, tile, tile);
      }
    }

    // anneaux ondulants en add
    p.blendMode(p.ADD);
    for (let i = 0; i < 8; i++) {
      const r =
        Math.min(p.width, p.height) *
        (0.12 + i * 0.12) *
        (0.9 + 0.1 * p.sin(t * (1 + i * 0.3)));
      const a = 40 - i * 4;
      p.fill((hueBase + i * 40) % 360, 100, 60, a);
      p.ellipse(
        p.width / 2 + p.sin(t * (1 + i * 0.2)) * 80,
        p.height / 2 + p.cos(t * (1 + i * 0.15)) * 60,
        r,
        r
      );
    }
    ...
    for (let i = 0; i < 90; i++) {
      const px = p.noise(t + i * 7.123) * p.width;
      const py = p.noise(t * 1.3 + i * 3.321) * p.height;
      const size = 1 + 6 * p.noise(i * 0.21 + t * 0.8);
      const hue = (hueBase + 180 * p.noise(i * 2.1 + t * 0.4)) % 360;
      p.fill(hue, 100, 100, 60 * p.noise(i * 0.5 + t * 0.9));
      p.circle(px, py, size);
    }

    // légère couche de fondu pour adoucir
    p.fill(0, 0, 0, 6);
